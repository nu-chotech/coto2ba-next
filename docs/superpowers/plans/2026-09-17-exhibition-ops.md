...
> ⚠️ **`0004_hint_cache_jsonb.sql` を適用した後は、API を 0004 以前のデプロイに戻さないこと。**
> `hint_cache.hints` が `text[]` から `jsonb` になっているので、**旧コードはヒントで 500 になる。**
> 事故ったときは戻すのではなく**前に進めること（roll forward）** ＝ 新しいデプロイで直す。
>
> ⚠️ **migrate の後にデプロイが失敗すると、ヒントだけが落ちる状態になる。**
> スキーマは新、コードは旧という組み合わせになるため。ゲーム自体は遊べるので気づきにくい。
> migrate を流したら、**デプロイが成功するまでを 1 つの作業として最後まで見届けること。**
...
**2026-09-18 に実測した材料（これを使うこと）**

`goal_pool.bot_moves` は**当てにならない**。旧ヒントアルゴリズム基準で計算されており、
主要候補がほぼ全部 6.0 で並んで難易度の区別になっていない（SPEC §6.2 の注記を参照）。

代わりに**ゴール語の「意味空間での近所」**を見る。近所に誰でも思いつく語が並んでいれば
直感的に解ける。並んでいなければ、来場者が真っ先に打つ語が効かない罠になる。

| ゴール | 意味空間の近所（近い順） | 判定 |
| --- | --- | --- |
| 土偶 | 縄文土器 / 縄文 / 土器 / 縄文時代 / 埴輪 / 出土 | **直感的** |
| 宝石 | 宝飾 / 指輪 / 真珠 / 貴金属 / ジュエリー / ネックレス | **直感的** |
| 望遠鏡 | 反射望遠鏡 / 電波望遠鏡 / 天体観測 / 天文台 / 天体 | **直感的** |
| 温泉 | 湯 / 温泉郷 / 露天風呂 / 日帰り温泉 / 温泉街 / 旅館 | **直感的**（ただし下記の注意） |
| 振り子 | 振子 / 回転 / バネ / 遠心力 / 車輪 / シーソー | まあ直感的 |
| 巫女 | 神官 / 女神 / 姫巫女 / 姫 / 魔女 / 精霊 | やや創作寄り |
| 琥珀 | 翡翠 / 雫 / 瞳 / 瑠璃 / 蒼 / 深紅 | **罠**（色・詩語） |
| 砂時計 | 鼓動 / 蒼い / 雨粒 / 天使 / かけら / 黄昏 | **罠**（歌詞） |
| 硝子 | 螺子 / 壜 / 紡 / 玻璃 / 真珠 / 琥珀 | **罠**（当て字） |
| 羅針盤 | コンパス / 道しるべ / 夜明け / 鼓動 / 黄昏 | **半分罠** |

**罠の正体**: 琥珀・砂時計・硝子・羅針盤は見た目が魅力的だが、Wikipedia では
**歌のタイトルとして大量に使われている**ため、意味空間では詩語のクラスタに落ちている。
来場者が「ガラス」「窓」と打っても近づかない。

実証: `硝子 ↔ 窓ガラス` の余弦距離は **0.6796**。これは `温泉 ↔ 琥珀`（0.6818）とほぼ同じ、
つまり**まったく無関係な語と同じくらい遠い**。

**注意が 2 つ**:
1. **`温泉` は `review_needed = true`** が立っている。展示に使う前に人が見ること
2. `土偶` / `望遠鏡` / `羅針盤` / `社殿` は**説明文が入っていない**（`description IS NULL`）。
   結果画面と図鑑のシートが寂しくなるので、Task 4 の説明バッチを先に流すか、手で入れる

**再実測のしかた**（ゴール候補を変えたくなったとき）:

```bash
docker exec coto2ba-pg psql -U coto2ba -d coto2ba -tA -F'|' -c "
WITH cand(w) AS (VALUES ('候補1'),('候補2'))
SELECT c.w, string_agg(n.word, ' / ' ORDER BY n.d)
FROM cand c CROSS JOIN LATERAL (
  SELECT v.word, (v.w2v <=> (SELECT w2v FROM vocab WHERE word = c.w)) AS d
  FROM vocab v WHERE v.is_output AND v.word <> c.w
  ORDER BY v.w2v <=> (SELECT w2v FROM vocab WHERE word = c.w) LIMIT 6
) n GROUP BY c.w;"
```
...
**2026-09-18 に実施済み（本番に適用）**

展示期間（10/01〜10/12）の盤面を手で選び直した:
...
**選定の根拠**: ゴールの「意味空間での近所」に**誰でも思いつく語が並ぶこと**を実測で確認した。
砂糖 → 小麦粉 / 糖蜜 / 香辛料 / 果物。大聖堂 → 聖堂 / 礼拝堂 / 教会。
空手 → 合気道 / 有段者 / 武道 / 剣道。細菌 → 菌 / バクテリア / 病原体 / 微生物。

**差し替えた理由**:
- 元の盤面は 14 日中 5 日で**スタート語が `is_concrete` を通っていなかった**
  （対処・発言・ビット・昔・強襲）。通っていても「本当」「久々」「瑞」「荘」のような
  混ぜようのない語が入っていた
- ゴール側も「生体」「自我」「盟主」が抽象的だった
- **10/11 のゴールが「感情」で、これは近傍に性的表現が並ぶ語だった**（下記）

**あわせてゴールプールから 5 語を無効化した（`enabled = false`）**:
`美少女` / `官能` / `女子アナ` / `お姉ちゃん` / `下品`。
NG 語リストは約 450 語でほぼ性的表現に偏っており、**この 5 語は `review_needed` を素通り**していた。
検出のしかたは「NG 語との余弦距離が 0.45 未満のゴール語を探す」:

```bash
URL=$(grep -m1 '^DATABASE_URL_DIRECT=' apps/api/.env.neon | cut -d= -f2- | tr -d '"')
docker run --rm pgvector/pgvector:pg17 psql "$URL" -c "
WITH ng(w) AS (VALUES ('ロリ'),('美乳'),('エロ'),('巨乳'),('痴漢'),('風俗'),('性的'),('裸'))
SELECT g.word, string_agg(DISTINCT n.w, ' / ')
FROM goal_pool g CROSS JOIN LATERAL (
  SELECT ng.w FROM ng WHERE EXISTS (SELECT 1 FROM vocab v WHERE v.word = ng.w)
   AND ((SELECT w2v FROM vocab WHERE word=g.word) <=> (SELECT w2v FROM vocab WHERE word=ng.w)) < 0.45
) n WHERE g.enabled GROUP BY g.word;"
```

**NG 語の種を増やして再実行すれば、同じやり方で追加の取りこぼしを探せる。**
...
**2026-09-18 に本番で実測した状況**:
- `goal_pool`: 説明あり **271 / 518 語**
- `word_descriptions`（ランタイム取得のキャッシュ）: **535 件すべて本文あり**

**目視レビューは全件やらなくてよい。** 下のクエリで候補を 19 件に絞れる。
Wikipedia の冒頭文は通常その見出し語で始まるので、**始まっていないものが取り違えの疑い**:

```bash
URL=$(grep -m1 '^DATABASE_URL_DIRECT=' apps/api/.env.neon | cut -d= -f2- | tr -d '"')
docker run --rm pgvector/pgvector:pg17 psql "$URL" -tA -F'|' -c "
SELECT word, left(text, 44) FROM word_descriptions
WHERE position(word in left(text, 20)) = 0 ORDER BY word;"
```

実測した 19 件の内訳は、**ほとんどが Wikipedia のリダイレクト**だった:

| 語 | 説明の冒頭 | 判定 |
| --- | --- | --- |
| プラスチック | 合成樹脂 とは… | リダイレクト。意味は合っている |
| リサイタル | 演奏会（えんそうかい）とは… | 同上 |
| 内装 | インテリア とは… | 同上 |
| 左側 | 左右（さゆう、ひだりみぎ）とは… | 同上 |
| 弟子 | 師弟（してい）とは… | 同上 |
| 金額 | 価格 とは… | 同上 |
| **提唱** | **提綱（ていこう）とは、仏教用語で…** | **取り違えの疑い。要確認** |

**やること**: 上のクエリを流し、リダイレクト（意味が合っている）と
取り違え（別の語の説明）を分ける。取り違えだけ `word_descriptions` を直接直すか、
その語をゴールプールから外す。**展示に出る文言なので人の目で見ること。**
...
- **端末の選定（2026-09-18 に実測）**

| 画面サイズ | 機種の例 | 判定 |
| --- | --- | --- |
| 414×896 以上 | iPhone 11 / XR / Plus 系、iPad | **推奨** |
| 390×844 | iPhone 12〜15 の標準 | 実用可（説明の長いお題のときだけ「混ぜる」に少しスクロール） |
| 375×667 | iPhone SE | **非推奨** |

**375×667 が非推奨なのは対戦のせいではない。** 1 人用のゲーム画面でも
「混ぜる」ボタンが画面外に出る（下端 712 > 667）。**来場者が主操作に届かない。**
対戦の人数は効かない（2 人でも 8 人でも同値）。

**ブースに SE クラスの端末を置かないこと。** 置くなら事前に実機で
「混ぜる」に指が届くかを確認すること。
