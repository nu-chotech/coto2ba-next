...
| | 値 |
| --- | --- |
| 1 手の応答（本番・単独） | 中央値 **247ms** |
| 負荷試験 5 req/s × 5 分（本番） | **エラー 0 / 中央値 247ms / p95 338ms / p99 712ms** |

SPEC §11.4 の負荷試験目標（p95 < 500ms、エラー 0）を達成済み（`python3 apps/api/tests/load.py <URL>`）。

途中で見つけて直した重大な問題:
- **HNSW インデックスが使われていなかった**（混合ベクトルを CTE 結合で作ると
  Postgres が定数と見なせず 100,853 行の全ソートに落ちる）。実測 220ms → 0.7ms
- **1 手の処理に競合状態**（同時リクエストが同じ seq を insert して 500）。
  games 行を FOR UPDATE でロックしたトランザクションに変更
- **起動直後の 401**（セッションが立つ前にリクエストが飛んでいた）
- **ゴールの近傍語を打つだけで 1 手クリアできた**（デイリーのランキングが成立しない）。
  ゲーム作成時にゴールの近傍 12 語を禁止リストに入れる
...
| 項目 | 状態 |
| --- | --- |
| ゴールプール `04_goal_pool.py` | **1 巡目完了**（easy 51 / normal 150 / hard 92）。具体名詞フィルタを足して 2 巡目を実行中（easy 59 / normal 194 / hard 115 まで到達） |
| モバイルの図鑑（SpaceCanvas）とシェア画像 | 実装中 |
